...
from svgtag.svg.layouts import brand_layout_auto, narcose_layout
...
layout = narcose_layout(width, height, padding=padding, title_ratio=title_ratio, side_ratio=side_ratio)

# =============================================================================
# 1. SHAPE + RECTO (titre + consignes + numéros)
# =============================================================================
shape_svg, layout = tablet_svg(
    width=width,
    height=height,
    padding=padding,
...
if size == 'large':
    ...
    add_text_zone(recto_svg, "Test de Narcose", font_path, title_area, n=1, text_scale=0.5)

# Consignes (texte latéral gauche)
side_area = layout.get_area('side_text')
if size == 'small':
    add_text_zone(recto_svg, "Test de Narcose", font_path, side_area, n=1, text_scale=0.6)
elif size == 'large':
    add_text_zone(recto_svg, "Maximum de chiffres en 2 minutes !\nAu fond et en surface, 1 pt si pas d'erreur.", font_path, side_area, n=2, text_scale=0.8)
...
brand_layout_obj, tw, th = brand_layout_auto(
    ...
    brand_width_scale=brand_width_scale,
    border=0,
    flip_axis=flip_axis,
)
...
add_text_zone(verso_svg, "Tetsudau", logo_font_path, brand_area)
...
verso_svg = verso_svg.flip_element(-1, axis=flip_axis, center=(center_x, center_y))

preview_A = recto_svg.copy()
preview_A.add_svg(shape_svg)
preview_A.generate_svg_file(str(output_path / "preview_A.svg"))

preview_B = verso_svg.copy()
preview_B.add_svg(shape_svg)
preview_B=preview_B.flip(axis=flip_axis, center=(center_x, center_y))
preview_B.recalculate_viewbox(margin=1)
preview_B.generate_svg_file(str(output_path / "preview_B.svg"))

# =============================================================================
# 3. SAUVEGARDER SVG (debug)
# =============================================================================
shape_svg.generate_svg_file(str(output_path / "shape.svg"))
recto_svg.generate_svg_file(str(output_path / "recto.svg"))
verso_svg.generate_svg_file(str(output_path / "verso.svg"))

print(f"✓ SVG sauvegardés:")
print(f"  - shape.svg (viewBox: {shape_svg.viewBox})")
print(f"  - recto.svg (viewBox: {recto_svg.viewBox})")
print(f"  - verso.svg (viewBox: {verso_svg.viewBox})")
...
logo_path2d = svg_to_path2d(verso_svg)
