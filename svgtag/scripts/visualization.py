...
def text_to_image(text: str, font_filepath: str, size: int, color: str, title: str):
    ...
    draw.text(
        xy=tuple(ti / 2 for ti in size), text=text, font=font, fill="black", anchor="mm"
    )
    image._exif = text
...
def random_coordinates(width, height, existing_coordinates, image):
    ...
    while any(
        x <= x2 + w and x + img_width >= x2 and y <= y2 + h and y + img_height >= y2
        for x2, y2, w, h in existing_coordinates
    ):
        x, y = (
            random.randint(0, width - img_width),
            random.randint(0, height - img_height),
        )
...
def place_images(images, background_path):
    ...
    coordinates = []
    for image in images:
        x, y, w, h = random_coordinates(
            background.width, background.height, coordinates, image
        )
...
def gen_table(data, font_title=[], text=[], width=2**10):
    ...
    for row in font_dict:
        ...
        image = text_to_image(
            text=txt,
            font_filepath=os.path.join(row["Path"], row["Name"]),
            size=(int((width - pad) / cols - pad), int((height - pad) / rows - pad)),
            color="#FFFFFF",
        )
        ...
        if font_title != []:
            draw.text(
                tuple(positions[i] - pad),
                row["Name"],
                fill=(0, 0, 0),
                font=ImageFont.truetype(
                    os.path.join(
                        font_list["Path"].loc[font_list["Name"] == font_title].values[0],
                        font_title,
                    ),
                    int(pad),
                ),
...
def main():
    ...
    for Style in Styles:
        ...
        gen_table(
            data=font_list.loc[font_list["Style"] == Style],
            font_title=[],
            text=[],
            width=2**10,
        )
    ...
    tab = {"Classics": fonts_c, "Nice": fonts_g, "Scripts": fonts_s, "All": fonts_a}
    ...
    for font in tab:
        ...
        # If dictionary:
        selec = font_list[font_list["Family"].isin(tab.get(font))].loc[
            font_list["Style"] == "Regular"
        ]
        # images = [text_to_image(text = ft.FT2Font('./static/fonts/' + f).family_name, font_filepath = './static/fonts/' + f, size=(2**10, 2**8), color='#F0FFFF') for f in selec.Name]
        images = [
            text_to_image(
                text="Example",
                font_filepath="./static/fonts/" + f,
                size=(2**10, 2**8),
                color="#F0FFFF",
                title=ft.FT2Font("./static/fonts/" + f).family_name,
            )
            for f in selec.Name
        ]
        n = len(images)
