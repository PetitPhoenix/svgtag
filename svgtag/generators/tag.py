    ...
    if outline == True:
        svg.add_element(
            "rect",
            {
                "x": x_txt,
                "y": y_txt,
                "width": width_txt - x_mm,
                "height": height_txt - y_mm,
                "stroke": "black",
                "fill": "transparent",
                "stroke-width": 0.1,
            },
        ...
        if border != None:
            svg.add_element(
                "rect",
                {
                    "x": x_txt - border,
                    "y": y_txt - border,
                    "width": width_txt + 2 * border - x_mm,
                    "height": height_txt + 2 * border - y_mm,
                    "stroke": "black",
                    "fill": "transparent",
                    "stroke-width": 0.1,
                },
...
def svg2stl(shape, thickness, output_path, side_A, side_B=None, brand=None):
    ...
    if side_B:
        ...
        if brand == True:
            ...
            )  # [0.6*(shape_mesh.extents[0] - side_B_mesh.extents[0]), 0.6*side_B_mesh.extents[1], thickness]))
        else:
            side_B_mesh = side_B_mesh.apply_transform(
                trimesh.transformations.scale_and_translate(
                    scale=[1, 1, 1], translate=[0, shape_mesh.extents[1], thickness]
                )
            )
...
def tag_3D(filename, input_path, output_path):
    ...
    logo_mesh = logo_mesh.apply_transform(
        trimesh.transformations.rotation_matrix(angle=np.pi, direction=[0, 1, 0])
    )
    logo_mesh = logo_mesh.apply_transform(
        trimesh.transformations.scale_and_translate(
            scale=[0.7, 0.7, 1], translate=[75, 10, 3]
        )
    )
    ...
    print(
        f"STL saved at {os.path.abspath(os.path.join(output_path, f'{filename}_X.stl'))}"
    )
...
def tag_3D_RV(recto, verso, input_path, output_path):
    ...
    verso_mesh = verso_mesh.apply_transform(
        trimesh.transformations.rotation_matrix(angle=np.pi, direction=[1, 0, 0])
    )
    verso_mesh = verso_mesh.apply_transform(
        trimesh.transformations.scale_and_translate(scale=[1, 1, 1], translate=[0, 35, 3])
    )
    ...
    print(
        f"STL saved at {os.path.abspath(os.path.join(output_path, f'{outputname}_X.stl'))}"
    )
