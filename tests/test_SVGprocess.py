...
class TestSVGprocess(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.output_path = os.path.join(os.path.dirname(__file__), 'outputs', 'SVGprocess')
        # Créer le répertoire s'il n'existe pas
        os.makedirs(self.output_path, exist_ok=True)
        # Chemin vers le fichier SVG source
        self.source_svg_path = os.path.join(os.path.dirname(__file__), '..', 'static', 'images', 'network.svg')
