...
def card_gen(text_elements, width_mm, height_mm, padding_mm, phi=None):
    ...
    for element in text_elements:
        font_path = (
            f"../static/fonts/{element['font'].replace('.ttf', '')}/{element['font']}"
        )
        svg_data = text_svg(
            text=element["text"],
            font_path=font_path,
...
def generate_participant_cards(
    name,
    title,
    discipline,
    location,
    ...
    if output_formats is not None:
        ...
        export.convert_svg_with_inkscape(
            svg_file_path, inkscape_path, output_formats, dpi
        )
...
if __name__ == "__main__":
    ...
    for index, row in data.iterrows():
        ...
        generate_participant_cards(
            ...
            prep_zip=True,
        )  # output_formats = ['png', 'pdf'] or None
